...
为实时字幕功能添加 **应用选择模式**，使用 macOS 14+ 的 `SCContentSharingPicker` API，让用户可以选择特定应用进行字幕。

---

## 问题背景

当前实现使用 `SCStream` 捕获全屏音频：
- 捕获**所有**系统音频，包括不相关的应用
- 用户不清楚哪些音频被捕获

使用 `SCContentSharingPicker` 的优势：
- **精准捕获**：只捕获用户选择的应用音频
- **系统级 UI**：权限引导更自然，用户明确知道在共享什么
- **隐私透明**：用户主动选择，减少"被监听"的疑虑

> ⚠️ **注意**：此功能仍需屏幕录制权限，仍会显示系统录屏图标。核心价值是"精准选择音频源"而非"避免权限"。
