---
description: 进入 Plan 模式，使用 mcp__sequential-thinking__sequentialthinking 生成并落地执行计划
...
2. 每次进入 Plan 模式时，**必须先做最小必要调研**，以现有实现与问题证据为基础再制定计划，禁止仅凭推理给出计划。
3. 每次进入 Plan 模式时，**必须调用已配置的 MCP 工具 `mcp__sequential-thinking__sequentialthinking`** 来分解任务，而不是直接在消息里即兴列计划。
4. 使用 `mcp__sequential-thinking__sequentialthinking` 时，应根据任务复杂度选择合适的思考步数 totalThoughts，并在需要时动态调整：
   - simple：totalThoughts 约 4 步；
   - medium：totalThoughts 约 7 步；
   - complex：totalThoughts 约 10 步（必要时可增加到 12 步左右）。
5. 你需要在思考结束后，整理出一份简洁、可执行的计划，并**默认尝试将计划落地到文件**（见「四、Plan 文件落地规范」）。
6. 如果受审批/策略限制无法写文件或调用 shell，要在回答中说明原因，并至少给出完整的计划文本。

**最小必要调研要求（Plan 前置）**：
- 读取最关键入口/文档（README、架构说明、主要入口文件或配置）。
- 使用 `rg`/定位关键符号，找出与任务直接相关的实现点与已知问题点。
- 明确现有测试入口/命令/依赖，记录可执行范围与缺口。
- 调研结果必须在计划中用 `file:line` 形式给出证据引用。
- 调研预算 5–8 次工具调用；超出需在计划中说明原因。

## 二、复杂度判断与 sequential-thinking 使用规范

在完成最小必要调研后，根据 `$ARGUMENTS` 与当前上下文给任务分级：
...
🔍 现状证据：
- <引用现有实现/问题点的 file:line 证据>

🧪 测试与验证路径：
- <现有测试入口/命令/依赖；若缺失则说明补齐策略>
...
🔍 现状与证据
- <基于调研的关键实现点/问题点，附 `file:line`>

🧪 测试与验证路径
- <可执行测试入口/命令/依赖；若缺失需说明补齐策略与最小可行验证>
```
