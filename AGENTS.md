...
### 10.2.1 Oracle Timeout Mitigation (CRITICAL)

Oracle runs on `gpt-5.5 (high)` with a **30-minute system timeout**. Large prompts (multi-file read + architecture design) routinely exceed this window. The model DOES produce output but the session closes before delivery.

**MANDATORY Oracle usage rules:**

| DO | DON'T |
|-----|-------|
| Give Oracle a **pre-summarized context** (Sisyphus reads files first, passes summary) | Ask Oracle to read 5+ files AND design architecture in one prompt |
| Split complex questions: "Review this design" (small) NOT "Design everything" | Combine research + design into one Oracle call |
| Use `metis` for pre-planning before Oracle | Use Oracle for first-pass analysis |
| Consider `ultrabrain` (xhigh variant, same model) for architecture that involves implementation | Use Oracle for implementation decisions (Oracle is read-only) |

**Oracle is best for**: Focused architecture review, debugging after failed attempts, pattern validation.
**Oracle is NOT for**: Reading many files from scratch, designing entire systems solo, open-ended exploration.

If Oracle times out **but produces content** (check working tree for changes), extract and commit the content — the session was productive, just slow to return.
...
### 10.6 Mandatory Post-Implementation Gates (NON-NEGOTIABLE)

Every completed phase or milestone MUST pass these gates before being declared "done":

| Gate | Command / Action | Frequency |
|------|-----------------|-----------|
| Formal Review | `skill("review-work")` → launch 5 parallel Oracle/QA agents | After EVERY phase/milestone |
| Roadmap Check | Read `.workflow/roadmap.md`, list ALL options, mark completed vs remaining | After EVERY phase |
| Full Test Suite | `swift test` (all pass) | After EVERY phase |
| Concurrency Check | `bash Tests/run-concurrency-check.sh` (0 warnings) | After EVERY phase |
| Subagent Audit | Review `task()` calls: correct `category` for domain? `visual-engineering` for UI? `explore` for code search? | Self-check per batch |

**BLOCKING RULE**: A phase is NOT complete until all 5 gates pass. "Tests pass" alone is insufficient.

### 10.7 Roadmap Back-Check Protocol

After completing any roadmap item:
1. Re-read `.workflow/roadmap.md` from disk
2. List ALL roadmap options with completion status
3. Report completed vs remaining items explicitly
4. Do NOT declare "all done" without confirming against the roadmap text

### 10.8 Delegation Audit Trail

For every `task()` call, the orchestrator implicitly records:
- Which `category` or `subagent_type` was used
- Whether it matched the domain (e.g., UI → `visual-engineering`, not `deep`)

**Self-audit checklist** (ask before declaring phase done):
| Question | Check |
|----------|-------|
| Was any UI/styling work delegated to non-`visual-engineering`? | If YES → BLOCKING |
| Was `explore`/`librarian` used for ALL read-only code searches? | If NO → anti-pattern |
| Was `oracle` consulted for architecture decisions? | If NO and complex → WARN |
| Were independent sub-tasks parallelized? | If NO → WARN |
| Did each `task()` prompt include all 6 template sections? | If NO → improve |

---

*Last updated: 2026-04-29 | Generated by ccg:init + Sisyphus delegation optimization + post-Phase5 self-review remediation + Oracle timeout guidance*
